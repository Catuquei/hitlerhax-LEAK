...
public final class TypeResolver
{
    ...
    private static Member getMemberRef(final Class<?> type) {
        Object constantPool;
        try {
            constantPool = TypeResolver.GET_CONSTANT_POOL.invoke(TypeResolver.JAVA_LANG_ACCESS, type);
        }
    ...
    static {
        ...
        try {
            ...
            if (TypeResolver.JAVA_VERSION < 9.0) {
                ...
                accessSetter = new AccessMaker() {
                    @Override
                    public void makeAccessible(final AccessibleObject object) throws Throwable {
                        overrideSetter.invokeWithArguments(object, true);
                    }
                };
            }
            final Method javaLangAccessGetter = sharedSecretsClass.getMethod("getJavaLangAccess", (Class<?>[])new Class[0]);
            accessSetter.makeAccessible(javaLangAccessGetter);
            TypeResolver.JAVA_LANG_ACCESS = javaLangAccessGetter.invoke(null, new Object[0]);
            ...
            accessSetter.makeAccessible(TypeResolver.GET_CONSTANT_POOL);
            accessSetter.makeAccessible(TypeResolver.GET_CONSTANT_POOL_SIZE);
            accessSetter.makeAccessible(TypeResolver.GET_CONSTANT_POOL_METHOD_AT);
            final Object constantPool = TypeResolver.GET_CONSTANT_POOL.invoke(TypeResolver.JAVA_LANG_ACCESS, Object.class);
